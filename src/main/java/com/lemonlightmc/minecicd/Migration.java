...
public class Migration {
    public static void migrate() throws IOException, InvalidConfigurationException {
        final int fileConfigVersion = MineCICD.config.getInt("version");

        final YamlConfiguration pluginConfig = new YamlConfiguration();
        pluginConfig.load(new InputStreamReader(Objects.requireNonNull(MineCICD.plugin.getResource("config.yml")),
                StandardCharsets.UTF_8));
        final int pluginConfigVersion = pluginConfig.getInt("version");
        ...
        if (fileConfigVersion < pluginConfigVersion) {
            if (fileConfigVersion < 20200) {
                ...
                MineCICD.log(
                        "Please change or regenerate your .gitignore file (See https://github.com/Konstanius/MineCICD/blob/master/src/main/resources/.gitignore)",
                        Level.SEVERE);
